...
/*** Rpt108RekapKIB_C  - 20022020 - Modified for Ver 2.0.7 [demi@simda.id] ***/
CREATE PROCEDURE dbo.Rpt108RekapKIB_C @Tahun varchar(4), @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(10), @D2 Datetime
WITH ENCRYPTION
AS
/*
...
--*/
	...
	SELECT J.Kd_UPBA, I.Nm_Provinsi, H.Nm_Kab_Kota,
		...
		dbo.fn_KdLokasi2_108(A.Kd_Pemilik, A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, @Kd_UPB) AS Kd_Lokasi,
		dbo.fn_KdLokasi2_108(A.Kd_Pemilik, A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, @Kd_UPB) AS Kd_Lokasi_Grp,
		...
		END AS Kondisi,
		A.Bertingkat_Tidak, A.Beton_Tidak, A.Luas_Lantai, A.Lokasi,
		A.Dokumen_Tanggal, A.Dokumen_Nomor, K.Luas_M2, A.Status_Tanah, 
	...
	FROM fn_Kartu108_BrgC(@D2, @Kd_Prov, @Kd_Kab_Kota, @Kd_Bidang, @Kd_Unit, @Kd_Sub, @Kd_UPB,'%','%','%','%','%','%','%','%',@JLap)  A INNER JOIN
		...
		Ta_KIB_A K ON A.Kd_Prov = K.Kd_Prov AND A.Kd_Kab_Kota = K.Kd_Kab_Kota AND A.Kd_Bidang = K.Kd_Bidang AND A.Kd_Unit = K.Kd_Unit AND A.Kd_Sub = K.Kd_Sub AND A.Kd_UPB = K.Kd_UPB AND A.Kd_Tanah1 = K.Kd_Aset1 
                              AND A.Kd_Tanah2 = K.Kd_Aset2 AND A.Kd_Tanah3 = K.Kd_Aset3 AND A.Kd_Tanah4 = K.Kd_Aset4 AND A.Kd_Tanah5 = K.Kd_Aset5 AND A.Kode_Tanah = K.No_Register /*LEFT OUTER JOIN
		(
		...
		WHERE (B.Tgl_SK <= (@Tahun + '1231')) AND (A.Kd_Aset1 = 3)
		) L ON A.Kd_Prov = L.Kd_Prov AND A.Kd_Kab_Kota = L.Kd_Kab_Kota AND A.Kd_Bidang = L.Kd_Bidang AND 
			A.Kd_Unit = L.Kd_Unit AND A.Kd_Sub = L.Kd_Sub AND A.Kd_UPB = L.Kd_UPB AND A.Kd_Aset1 = L.Kd_Aset1 AND A.Kd_Aset2 = L.Kd_Aset2 AND 
			A.Kd_Aset3 = L.Kd_Aset3 AND A.Kd_Aset4 = L.Kd_Aset4 AND A.Kd_Aset5 = L.Kd_Aset5 AND A.No_Register = L.No_Register*/,
		(
