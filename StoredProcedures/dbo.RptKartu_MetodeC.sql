...
CREATE PROCEDURE [dbo].[RptKartu_MetodeC] @Tahun varchar(4), @Kd_Pemilik Varchar(3), @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(3),
...
DECLARE @Metode Table(Tahun varchar(4), Kd_Aset1 tinyint, Kd_Aset2 tinyint, Kd_Aset3 tinyint, Kd_Aset4 tinyint, Status_Manfaat Varchar(100), Metode tinyint, Nama_Metode varchar(50), Umur tinyint)

INSERT INTO @Metode
SELECT	@Tahun AS Tahun, B.Kd_Aset1, B.Kd_Aset2, B.Kd_Aset3, B.Kd_Aset4, A.Status_Manfaat, B.Metode, B.Nama_Metode, B.Umur
FROM
	(
	...
	WHERE	(A.Kd_Riwayat BETWEEN 8 AND 13) AND (Kd_Id = (SELECT MAX(Kd_Id) FROM Ta_KIBCR WHERE Kd_Riwayat BETWEEN 8 AND 13))
	) A RIGHT OUTER JOIN
	(
	SELECT	A.Tahun, D.Kd_Prov, D.Kd_Kab_Kota, D.Kd_Bidang, D.Kd_Unit, D.Kd_Sub, D.Kd_UPB, A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Metode, A.Nama_Metode,
		CASE
	...
	FROM
		...
		Ta_KIB_C B ON A.Kd_Aset1 = B.Kd_Aset1 AND A.Kd_Aset2 = B.Kd_Aset2 AND A.Kd_Aset3 = B.Kd_Aset3 AND A.Kd_Aset4 = B.Kd_Aset4 LEFT OUTER JOIN
		Ta_KIBCR D ON B.Kd_Prov = D.Kd_Prov AND B.Kd_Kab_Kota = D.Kd_Kab_Kota AND B.Kd_Bidang = D.Kd_Bidang AND B.Kd_Unit = D.Kd_Unit AND 
...
SELECT	A.Metode, A.Status_Manfaat, A.Nama_Metode, A.Umur
FROM	@Metode A
