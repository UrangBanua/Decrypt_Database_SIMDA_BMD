...
/*** RptKIR - 30012016 - Modified for Ver 2.0.7 [demi@simda.id] ***/
CREATE PROCEDURE RptKIR @Tahun varchar(4), @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(3), @Kd_Ruang varchar(3), @D2 Datetime
WITH ENCRYPTION
AS
 ...
 */
	...
	FROM
		...
		SELECT A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, 
			...
			ELSE A.Kd_Ruang
			END AS Kd_Ruang, 
			A.Kd_Pemilik, A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5,
			A.Merk, A.Type, A.CC AS Ukuran, A.Bahan, YEAR(A.Tgl_Perolehan) AS Tahun, YEAR(A.Tgl_Pembukuan) AS Tahun_Buku , A.Nomor_Pabrik, SUM(A.Harga) AS Harga, COUNT(*) AS Jml,
			SUM(CASE A.Kondisi
			WHEN 1 THEN 1
			ELSE 0
			END) AS Jml_B,
		...
		WHERE (A.Kd_Prov = @Kd_Prov) AND (A.Kd_Kab_Kota = @Kd_Kab_Kota) AND (A.Kd_Bidang LIKE @Kd_Bidang) AND (A.Kd_Unit LIKE @Kd_Unit) AND (A.Kd_Sub LIKE @Kd_Sub) AND (A.Kd_UPB LIKE @Kd_UPB) 
		      AND (A.Kd_Aset2 > 3) 
		GROUP BY A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, A.Kd_Ruang, A.Kd_Pemilik, A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5,
		...
		SELECT A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, 
			...
			A.Judul, A.Pencipta, 
			CASE A.Kd_Aset2
			WHEN 19 THEN CONVERT(varchar, A.Ukuran) + CASE ISNULL(A.Bahan, '') WHEN '' THEN '' ELSE ' ' + A.Bahan END
		...
		GROUP BY A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, A.Kd_Ruang, A.Kd_Pemilik, A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5,
			...
			WHEN 19 THEN ''
			ELSE A.Bahan
			END, YEAR(A.Tgl_Perolehan), YEAR (A.Tgl_Pembukuan), ISNULL(A.Keterangan, ''), C.Kd_Ruang, C.Tgl_Dokumen
		) A INNER JOIN
