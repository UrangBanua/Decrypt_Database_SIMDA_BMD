...
/*** RptRincianPemilik - 05012016 - Modified for Ver 2.0.7 [demi@simda.id] ***/
CREATE PROCEDURE dbo.Rpt108RincianPemilik @Tahun varchar(4), @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(3), @Pemilik varchar(3), @D2 Datetime
WITH ENCRYPTION
AS
/*
DECLARE @Tahun varchar(4), @D2 Datetime, @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(3), @Pemilik varchar(3)
SET @Tahun = '2019'
SET @D2 = '20191231'
SET @Kd_Prov = '19'
SET @Kd_Kab_Kota = '00'
...
*/
	...
	--14	Pemerintah Kabupaten/Kota Lain
	--15	Non Pemerintah
...
AND (A.Kd_Sub LIKE @Kd_Sub) AND (A.Kd_UPB LIKE @Kd_UPB)
	...
	SELECT J.Kd_BidangA, J.Kd_UnitA, J.Kd_SubA, J.Kd_UPBA, I.Nm_Provinsi, H.Nm_Kab_Kota, 
		J.Nm_Bidang_Gab, J.Nm_Unit_Gab, J.Nm_Sub_Unit_Gab, J.Nm_UPB_Gab, K.Nm_Pemilik,
		...
		RIGHT('00' + CONVERT(varchar, A.Kd_Aset1),2) + ' . ' + RIGHT('00' + CONVERT(varchar, A.Kd_Aset2),2)+ ' . ' + RIGHT('00' + CONVERT(varchar, A.Kd_Aset3),2)+ ' . ' + 
RIGHT('00' + CONVERT(varchar, A.Kd_Aset4),3) AS Kd_Gab4,
		...
		CASE 
		--WHEN (ISNULL(A.Alamat, '') <> '') THEN A.Alamat
	...
	FROM
		(
		SELECT A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Aset, A.Kd_Aset0, A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5, A.Kd_Pemilik, 
		...
		GROUP BY A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Aset, A.Kd_Aset0, A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5, A.Kd_Pemilik,
			A.Tahun, A.Merk, A.Type, A.No_Sertifikat, A.Bahan, A.Asal_Usul, A.Ukuran, A.Satuan, A.Kondisi, A.Keterangan, A.Lokasi
		) A INNER JOIN
		...
		Ref_Rek4_108 C ON B.Kd_Aset = C.Kd_Aset AND B.Kd_Aset0 = C.Kd_Aset0 AND B.Kd_Aset1 = C.Kd_Aset1 AND B.Kd_Aset2 = C.Kd_Aset2 AND B.Kd_Aset3 = C.Kd_Aset3 AND B.Kd_Aset4 = C.Kd_Aset4 INNER JOIN
	...
	ORDER BY A.Kd_Pemilik, Kd_Gab_Brg, A.Tahun, No_Register
GO
