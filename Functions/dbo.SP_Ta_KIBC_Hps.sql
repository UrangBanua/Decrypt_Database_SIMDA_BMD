...
/*** SP_Ta_KIBC_Hps - 02032017 - Modified for Ver 2.0.7 [demi@simda.id] ***/
CREATE PROCEDURE SP_Ta_KIBC_Hps @Tahun varchar(4), @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(3),
...
*/
	...
	SELECT A.IDPemda, A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, 
		...
		B.Kondisi, ISNULL(B.Harga,A.Harga) AS Harga, A.Harga AS Harga_A, A.Masa_Manfaat, A.Nilai_Sisa, A.Keterangan, A.Tahun, A.No_SP2D, A.No_Id, A.Kd_Kecamatan, A.Kd_Desa, A.Invent,
		A.No_SKGuna, A.Kd_Penyusutan, A.Kd_Data, A.Kd_KA, A.Log_User, A.Log_entry,
	...
	FROM Ta_KIB_C A LEFT OUTER JOIN
		fn_Kartu_BrgC(@D2, @Kd_Prov, @Kd_Kab_Kota, @Kd_Bidang, @Kd_Unit, @Kd_Sub, @Kd_UPB, @Kd_Aset1, @Kd_Aset2, @Kd_Aset3, @Kd_Aset4, @Kd_Aset5, @Reg, @JLap) B ON
