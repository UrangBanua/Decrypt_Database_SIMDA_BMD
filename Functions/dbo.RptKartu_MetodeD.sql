...
CREATE PROCEDURE [dbo].[RptKartu_MetodeD] @Tahun varchar(4), @Kd_Pemilik Varchar(3), @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(3),
...
FROM
	...
	FROM
		...
		Ta_KIB_D B ON A.Kd_Aset1 = B.Kd_Aset1 AND A.Kd_Aset2 = B.Kd_Aset2 AND A.Kd_Aset3 = B.Kd_Aset3 AND A.Kd_Aset4 = B.Kd_Aset4 LEFT OUTER JOIN
		Ta_KIBDR D ON B.Kd_Prov = D.Kd_Prov AND B.Kd_Kab_Kota = D.Kd_Kab_Kota AND B.Kd_Bidang = D.Kd_Bidang AND B.Kd_Unit = D.Kd_Unit AND 
