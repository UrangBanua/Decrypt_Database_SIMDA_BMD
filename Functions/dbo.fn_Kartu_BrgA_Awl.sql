...
/*** fn_Kartu_BrgA_Awl - 16112015 - Modified for Ver 2.0.7 [demi@simda.id] ***/

CREATE FUNCTION [dbo].[fn_Kartu_BrgA_Awl] (@D1 Datetime, @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(4),
...
DECLARE @tmpHarga TABLE(Kd_ID int, Kd_Riwayat tinyint, Kd_Prov tinyint, Kd_Kab_Kota tinyint, Kd_Bidang tinyint, Kd_Unit smallint, Kd_Sub smallint, Kd_UPB int, 
			Kd_ASet1 tinyint, Kd_Aset2 tinyint, Kd_Aset3 tinyint, Kd_Aset4 tinyint, Kd_Aset5 tinyint, No_Register int, Harga money, Luas_M2 Float(8), Kd_Data tinyint, Kd_KA Tinyint)
...
--4. KAPITALISASI
	...
	SELECT	MAX(A.Kd_ID)AS Kd_ID, A.Kd_Riwayat, A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, 
		A.Kd_ASet1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5, A.No_Register, ISNULL(SUM(A.Harga),0) AS Harga, ISNULL(SUM(A.Luas_M2),0) AS Luas_M2, A.Kd_Data, NULL AS KD_KA
	FROM	Ta_KIBAR A 
...
--5. HAPUS SEBAGIAN
	INSERT INTO @tmpHarga
	SELECT	MAX(A.Kd_ID)AS Kd_ID, A.Kd_Riwayat, A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, 
		A.Kd_ASet1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5, A.No_Register, ISNULL(SUM(-Harga),0) AS Harga, ISNULL(SUM(A.Luas_M2),0) AS Luas_M2, A.Kd_Data, NULL AS Kd_KA
	FROM	Ta_KIBAR A
	WHERE	A.Tgl_Dokumen <= @D1 AND A.Kd_Prov = @Kd_Prov AND A.Kd_Kab_Kota = @Kd_Kab_Kota AND A.Kd_Bidang LIKE @Kd_Bidang AND A.Kd_Unit LIKE @Kd_Unit AND A.Kd_Sub LIKE @Kd_Sub AND A.Kd_UPB LIKE @Kd_UPB AND
...
--UBAH DATA KIB A (RIWAYAT)
   ...
   SELECT A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, /*C.Kd_Gab_Bidang, C.Kd_Gab_Unit, C.Kd_Gab_Sub, C.Kd_Gab_UPB,*/ A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5, A.No_Register,  A.Harga, A.Luas_M2,
   ...
   FROM (
	...
	SELECT A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, A.Kd_ASet1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5, A.No_Register,  SUM(A.Harga)AS Harga, SUM(A.Luas_M2) AS Luas_M2, MAX(A.Kd_KA) AS Kd_KA
	FROM @TMPHARGA A 
