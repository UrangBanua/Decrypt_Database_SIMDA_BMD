...
/*** fn_Kartu108_BrgF - 14032019 - Modified for v3.0.0 [demi@simda.id] ***/
CREATE FUNCTION [dbo].[fn_Kartu108_BrgF] (@D2 Datetime, @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(4),
...
RETURNS @UpdateKIB TABLE(IDPemda varchar(17), Kd_Prov tinyint, Kd_Kab_Kota tinyint, Kd_Bidang tinyint, Kd_Unit smallint, Kd_Sub smallint, Kd_UPB int,
			 ...
			 Lokasi varchar(255), Dokumen_Tanggal datetime, Dokumen_Nomor varchar(100), Status_Tanah varchar(50), Kd_Tanah1 tinyint,
...
*/
	...
	FROM
        (
		SELECT	A.IDPemda, A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, A.Kd_Aset8, A.Kd_Aset80, A.Kd_Aset81, A.Kd_Aset82, A.Kd_Aset83, A.Kd_Aset84, A.Kd_Aset85, A.No_Reg8, 
			...
			A.Asal_Usul, 1 AS Kondisi, A.Harga, 
		...
		WHERE	(Kd_Prov = @Kd_Prov) AND (Kd_Kab_Kota = @Kd_Kab_Kota) AND (Kd_Bidang LIKE @Kd_Bidang) AND (Kd_Unit LIKE @Kd_Unit) AND (Kd_Sub LIKE @Kd_Sub) AND (Kd_UPB LIKE @Kd_UPB) 
			AND (A.Kd_Data = 3) AND (A.Tgl_Pembukuan <= @D2)
		...
		SELECT	A.IDPemda, A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, A.Kd_Aset8, A.Kd_Aset80, A.Kd_Aset81, A.Kd_Aset82, A.Kd_Aset83, A.Kd_Aset84, A.Kd_Aset85, A.No_Reg8, 
			...
			'' AS Status_Tanah, '' AS Kd_Tanah1, '' AS Kd_Tanah2, '' AS Kd_Tanah3, '' AS Kd_Tanah4, '' AS Kd_Tanah5, '' AS Kd_Tanah, 
			A.Asal_Usul, A.Kondisi AS Kondisi, A.Harga, 
                        CASE WHEN @JLap = 1 THEN '' ELSE A.Keterangan END AS Keterangan,
                        A.Kd_Data, A.Kd_KA
		FROM	Ta_KIB_E A
		WHERE	(Kd_Prov = @Kd_Prov) AND (Kd_Kab_Kota = @Kd_Kab_Kota) AND (Kd_Bidang LIKE @Kd_Bidang) AND (Kd_Unit LIKE @Kd_Unit) AND (Kd_Sub LIKE @Kd_Sub) AND (Kd_UPB LIKE @Kd_UPB) 
		...
		FROM	Ta_KIBAR A
		WHERE	(Kd_Prov = @Kd_Prov) AND (Kd_Kab_Kota = @Kd_Kab_Kota) AND (Kd_Bidang LIKE @Kd_Bidang) AND (Kd_Unit LIKE @Kd_Unit) AND (Kd_Sub LIKE @Kd_Sub) AND (Kd_UPB LIKE @Kd_UPB) 
			AND (A.Kd_Data = 3) AND (A.Tgl_Pembukuan <= @D2) AND (A.Tgl_Dokumen > @D2)
