...
/*** RptRekapBI - 05092017 - Modified for Ver 2.0.7 [demi@simda.id] ***/
CREATE PROCEDURE dbo.RptRekapBI @Tahun varchar(4), @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(10), @D2 Datetime
WITH ENCRYPTION
AS
/*
...
SET @Kd_UPB = ''
*/
	DECLARE @tmpBI TABLE(Kd_Prov tinyint, Kd_Kab_Kota tinyint, Kd_Bidang smallint, Kd_Unit smallint, Kd_Sub smallint, Kd_UPB int, Kd_Aset1 tinyint, Kd_Aset2 tinyint, Kd_Aset3 tinyint, Kd_Aset4 tinyint, Kd_Aset5 tinyint, Kd_Pemilik tinyint, Tgl_Perolehan datetime, Harga money)
	...
	SELECT J.Kd_UPBA, I.Nm_Provinsi, H.Nm_Kab_Kota,
		...
		dbo.fn_KdLokasi(A.Kd_Pemilik, A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, @Kd_UPB, A.Tahun) AS Kd_Lokasi,
		A.Kd_Aset1, A.Kd_Aset2, RIGHT('0' + CONVERT(varchar, A.Kd_Aset1), 2) AS Kd_Gab1, RIGHT('0' + CONVERT(varchar, A.Kd_Aset2), 2) AS Kd_Gab2,
	...
	FROM
		...
		SELECT Kd_Prov, Kd_Kab_Kota, Kd_Bidang, Kd_Unit, Kd_Sub,
			Kd_Aset1, Kd_Aset2, Kd_Pemilik, YEAR(Tgl_Perolehan) AS Tahun, COUNT(*) AS Total, SUM(Harga) AS Harga
		FROM @tmpBI
		GROUP BY Kd_Prov, Kd_Kab_Kota, Kd_Bidang, Kd_Unit, Kd_Sub, Kd_Aset1, Kd_Aset2, Kd_Pemilik, YEAR(Tgl_Perolehan)
		) A INNER JOIN
		Ref_Rek_Aset2 B ON A.Kd_Aset1 = B.Kd_Aset1 AND A.Kd_Aset2 = B.Kd_Aset2 INNER JOIN
		Ref_Rek_Aset1 K ON B.Kd_Aset1 = K.Kd_Aset1 INNER JOIN
	...
	ORDER BY Kd_Lokasi, A.Kd_Aset1, A.Kd_Aset2
