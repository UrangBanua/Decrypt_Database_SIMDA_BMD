...
CREATE PROCEDURE dbo.Rpt108KIB_B @Tahun varchar(4), @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(3), @D2 Datetime
WITH ENCRYPTION
AS
/*
...
--*/
	...
	DECLARE @Ta_KIB_B TABLE(Kd_Prov tinyint, Kd_Kab_Kota tinyint, Kd_Bidang tinyint, Kd_Unit smallint, Kd_Sub smallint, Kd_UPB smallint, Kd_Pemilik tinyint, Kd_KA tinyint, Kd_Aset tinyint, Kd_Aset0 tinyint, Kd_Aset1 tinyint, Kd_Aset2 tinyint, Kd_Aset3 tinyint, Kd_Aset4 tinyint, Kd_Aset5 int, Reg1 int, Reg2 int, MerkType varchar(100), CC varchar(50), Bahan varchar(50), Tahun smallint, Nomor_Pabrik varchar(50), Nomor_Rangka varchar(50), Nomor_Mesin varchar(50), Nomor_Polisi varchar(10), Nomor_BPKB varchar(50), Asal_usul varchar(50), Harga money, Total int, Keterangan varchar(255))
	DECLARE @Kd_Prov1 tinyint, @Kd_Kab_Kota1 tinyint, @Kd_Bidang1 tinyint, @Kd_Unit1 smallint, @Kd_Sub1 smallint, @Kd_UPB1 smallint, @Kd_Pemilik1 tinyint, @Kd_KA1 tinyint, @Kd_Aset1 tinyint, @Kd_Aset01 tinyint, @Kd_Aset11 tinyint, @Kd_Aset21 tinyint, @Kd_Aset31 tinyint, @Kd_Aset41 tinyint, @Kd_Aset51 int, @No_Register1 int, @MerkType1 varchar(100), @CC1 varchar(50), @Bahan1 varchar(50), @Tahun1 smallint, @Nomor_Pabrik1 varchar(50), @Nomor_Rangka1 varchar(50), @Nomor_Mesin1 varchar(50), @Nomor_Polisi1 varchar(10), @Nomor_BPKB1 varchar(50), @Asal_usul1 varchar(50), @Harga1 money, @Keterangan1 varchar(255)
	DECLARE @Kd_Prov2 tinyint, @Kd_Kab_Kota2 tinyint, @Kd_Bidang2 tinyint, @Kd_Unit2 smallint, @Kd_Sub2 smallint, @Kd_UPB2 smallint, @Kd_Pemilik2 tinyint, @Kd_KA2 tinyint, @Kd_Aset2 tinyint, @Kd_Aset02 tinyint, @Kd_Aset12 tinyint, @Kd_Aset22 tinyint, @Kd_Aset32 tinyint, @Kd_Aset42 tinyint, @Kd_Aset52 int, @MerkType2 varchar(200), @CC2 varchar(50), @Bahan2 varchar(50), @Tahun2 smallint, @Nomor_Pabrik2 varchar(50), @Nomor_Rangka2 varchar(50), @Nomor_Mesin2 varchar(50), @Nomor_Polisi2 varchar(20), @Nomor_BPKB2 varchar(50), @Asal_usul2 varchar(50), @Keterangan2 varchar(255)
	...
	SELECT A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, A.Kd_Pemilik, A.Kd_KA ,
		...
		ELSE '-'
		END AS MerkType, ISNULL(A.CC, '') AS CC, ISNULL(A.Bahan, '') AS Bahan, YEAR(A.Tgl_Perolehan) AS Tahun,
		ISNULL(A.Nomor_Pabrik, '') AS Nomor_Pabrik, ISNULL(A.Nomor_Rangka, '') AS Nomor_Rangka,
		ISNULL(A.Nomor_Mesin, '') AS Nomor_Mesin, ISNULL(A.Nomor_Polisi, '') AS Nomor_Polisi, ISNULL(A.Nomor_BPKB, '') AS Nomor_BPKB, ISNULL(A.Asal_Usul, '') AS Asal_Usul, A.Harga, ISNULL(A.Keterangan, '') AS Keterangan
	FROM fn_Kartu108_BrgB2(@D2, @Kd_Prov, @Kd_Kab_Kota, @Kd_Bidang, @Kd_Unit, @Kd_Sub, @Kd_UPB,'%','%','%','%','%','%','%','%',@JLap)  A 
	WHERE (A.Kd_Prov = @Kd_Prov) AND (A.Kd_Kab_Kota = @Kd_Kab_Kota) AND (A.Kd_Bidang LIKE @Kd_Bidang) AND (A.Kd_Unit LIKE @Kd_Unit) AND (A.Kd_Sub LIKE @Kd_Sub) AND (A.Kd_UPB LIKE @Kd_UPB)
		--AND (A.Tahun = @Tahun)
		--AND A.Kd_Aset = 1 AND A.Kd_Aset0 = 3 AND A.Kd_Aset1 = 2 AND A.Kd_Aset2 = 2 AND A.Kd_Aset3 = 1 AND A.Kd_Aset4 = 2 AND A.Kd_Aset5 = 3
	OPEN c1

	FETCH NEXT FROM c1 INTO @Kd_Prov1, @Kd_Kab_Kota1, @Kd_Bidang1, @Kd_Unit1, @Kd_Sub1, @Kd_UPB1, @Kd_Pemilik1, @Kd_KA1 ,
							@Kd_Aset1, @Kd_Aset01, @Kd_Aset11, @Kd_Aset21, @Kd_Aset31, @Kd_Aset41, @Kd_Aset51, @No_Register1, 
							@MerkType1, @CC1, @Bahan1, @Tahun1, @Nomor_Pabrik1, @Nomor_Rangka1, 
							@Nomor_Mesin1, @Nomor_Polisi1, @Nomor_BPKB1, @Asal_usul1, @Harga1, @Keterangan1
	...
	SET @Kd_Aset2 = @Kd_Aset1
	SET @Kd_Aset02 = @Kd_Aset01
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @Kd_Prov2, @Kd_Kab_Kota2, @Kd_Bidang2, @Kd_Unit2, @Kd_Sub2, @Kd_UPB2, @Kd_Pemilik2, @Kd_KA2, 
					@Kd_Aset2, @Kd_Aset02, @Kd_Aset12, @Kd_Aset22, @Kd_Aset32, @Kd_Aset42, @Kd_Aset52, @Reg1, @Reg2, 
					@MerkType2, @CC2, @Bahan2, @Tahun2, @Nomor_Pabrik2, @Nomor_Rangka2, @Nomor_Mesin2, @Nomor_Polisi2, 
					@Nomor_BPKB2, @Asal_usul2, @Harga2, @Jumlah, @Keterangan2
		...
		FETCH NEXT FROM c1 INTO @Kd_Prov1, @Kd_Kab_Kota1, @Kd_Bidang1, @Kd_Unit1, @Kd_Sub1, @Kd_UPB1, @Kd_Pemilik1, @Kd_KA1, @Kd_Aset1, @Kd_Aset01, @Kd_Aset11, @Kd_Aset21, @Kd_Aset31, @Kd_Aset41, @Kd_Aset51, @No_Register1, @MerkType1, @CC1, @Bahan1, @Tahun1, @Nomor_Pabrik1, @Nomor_Rangka1, @Nomor_Mesin1, @Nomor_Polisi1, @Nomor_BPKB1, @Asal_usul1, @Harga1, @Keterangan1
	END

	INSERT INTO @Ta_KIB_B
	SELECT @Kd_Prov2, @Kd_Kab_Kota2, @Kd_Bidang2, @Kd_Unit2, @Kd_Sub2, @Kd_UPB2, @Kd_Pemilik2, @Kd_KA2, @Kd_Aset2, @Kd_Aset02, @Kd_Aset12, @Kd_Aset22, @Kd_Aset32, @Kd_Aset42, @Kd_Aset52, @Reg1, @Reg2, @MerkType2, @CC2, @Bahan2, @Tahun2, @Nomor_Pabrik2, @Nomor_Rangka2, @Nomor_Mesin2, @Nomor_Polisi2, @Nomor_BPKB2, @Asal_usul2, @Harga2, @Jumlah, @Keterangan2

	CLOSE c1
	DEALLOCATE c1

/*
SELECT * FROM @Ta_KIB_B
WHERE Kd_Bidang =@Kd_Bidang AND Kd_Unit =@Kd_Unit
AND Tahun = @Tahun
*/

	SELECT J.Kd_UPBA, I.Nm_Provinsi, H.Nm_Kab_Kota,
		F.Nm_Bidang, E.Nm_Unit, D.Nm_Sub_Unit, J.Nm_UPB_Gab,
		dbo.fn_KdLokasi_108(A.Kd_Pemilik, A.Kd_KA, A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, A.Tahun) AS Kd_Lokasi,
		dbo.fn_KdLokasi3_108(A.Kd_Pemilik, A.Kd_KA, A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, A.Tahun) AS Kd_Lokasi_Grp,
		B.Nm_Aset5,
		REPLACE(dbo.fn_GabBarang_108(A.Kd_Aset,A.Kd_Aset0,A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5), ' . ', '.') AS Kd_Gab_Brg,
		--CONVERT(varchar, A.Kd_Aset) + '.' + CONVERT(varchar, A.Kd_Aset0) + '.' + CONVERT(varchar, A.Kd_Aset1) + '.' + CONVERT(varchar, A.Kd_Aset2) + '.' + CONVERT(varchar, A.Kd_Aset3) + '.' + RIGHT ('00' + CONVERT(varchar, A.Kd_Aset4),2) + '.' + RIGHT ('00' + CONVERT(varchar, A.Kd_Aset5),2)  AS Kd_Gab_Brg,
		CASE WHEN A.Total = 1 THEN RIGHT('000000' + CONVERT(varchar, A.Reg1), 6)
		...
		, CASE WHEN A.Kd_KA = 1 THEN 'Intra'
			   --WHEN A.Kd_KA = 2 THEN 'Extra'
			   ELSE 'Extra'
			   END AS KA
	...
	FROM @Ta_KIB_B A INNER JOIN
		...
		FROM Ta_UPB A INNER JOIN
			...
			WHERE (A.Kd_Aset = 1 AND A.Kd_Aset0 = 3 AND A.Kd_Aset1 = 2) AND A.Total >= 1 
	ORDER BY Kd_KA DESC, Kd_Lokasi_Grp, Kd_Gab_Brg, No_Register
