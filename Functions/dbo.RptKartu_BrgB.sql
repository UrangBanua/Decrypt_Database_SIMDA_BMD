...
/*** RptKartu_BrgB - 12122015 - Modified for Ver 2.0.7 [demi@simda.id] ***/
CREATE PROCEDURE [dbo].[RptKartu_BrgB] @Tahun varchar(4), @Kd_Pemilik Varchar(3), @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(3),
...
*/
	...
	FROM	
		...
		WHERE	(A.Kd_Riwayat BETWEEN 8 AND 13) AND (Kd_Id = (SELECT MAX(Kd_Id) FROM Ta_KIBBR WHERE Kd_Riwayat BETWEEN 8 AND 13))
		...
		(SELECT	A.Tahun, A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Metode, A.Nama_Metode, B.Kd_Ruang, C.Nm_Ruang,
			CASE
		...
		FROM
			...
			Ta_KIB_B B ON A.Kd_Aset1 = B.Kd_Aset1 AND A.Kd_Aset2 = B.Kd_Aset2 AND A.Kd_Aset3 = B.Kd_Aset3 AND A.Kd_Aset4 = B.Kd_Aset4 LEFT OUTER JOIN
			...
			Ta_KIBBR D ON B.Kd_Prov = D.Kd_Prov AND B.Kd_Kab_Kota = D.Kd_Kab_Kota AND B.Kd_Bidang = D.Kd_Bidang AND B.Kd_Unit = D.Kd_Unit AND 
