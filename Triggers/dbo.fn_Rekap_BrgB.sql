...
/*** fn_Kartu_BrgB - 16112015 - Modified for Ver 2.0.7 [demi@simda.id] ***/

CREATE FUNCTION [dbo].[fn_Rekap_BrgB] (@D2 Datetime, @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(4))
...
*/
    ...
    INSERT INTO @no_urut
    SELECT MAX(Kd_Id) AS Kd_Id, IDPemda
    FROM Ta_KIBBR
...
--2. PEROLEHAN 2 
	...
	FROM	Ta_KIBBR A
	WHERE	Tgl_dokumen > @D2 AND Kd_Riwayat = 3 AND Tgl_Pembukuan <= @D2 AND Kd_Prov = @Kd_Prov AND Kd_Kab_Kota = @Kd_Kab_Kota AND Kd_Bidang LIKE @Kd_Bidang AND Kd_Unit LIKE @Kd_Unit AND Kd_Sub LIKE @Kd_Sub AND Kd_UPB LIKE @Kd_UPB
		AND A.Kd_Bidang <> 22
...
--3. PEROLEHAN 3 
	...
	FROM Ta_KIBBHapus A
	WHERE Tgl_SK > @D2 AND Tgl_Pembukuan <= @D2 AND Kd_Prov = @Kd_Prov AND Kd_Kab_Kota = @Kd_Kab_Kota AND Kd_Bidang LIKE @Kd_Bidang AND Kd_Unit LIKE @Kd_Unit AND Kd_Sub LIKE @Kd_Sub AND Kd_UPB LIKE @Kd_UPB
	GROUP BY A.IDPemda, A.Kd_ASet1, A.Kd_Aset2, Kd_KA

--4. KAPITALISASI
	INSERT INTO @tmpHarga
	SELECT A.IDPemda, A.Kd_ASet1, A.Kd_Aset2, ISNULL(SUM(A.Harga),0) AS Harga, NULL AS Kd_Data
	FROM	Ta_KIBBR A 
...
--5. HAPUS SEBAGIAN
	INSERT INTO @tmpHarga
	SELECT	A.IDPemda, A.Kd_ASet1, A.Kd_Aset2, ISNULL(SUM(-A.Harga),0) AS Harga, NULL
	FROM	Ta_KIBBR A
	WHERE	A.Tgl_dokumen <= @D2 AND A.Kd_Prov = @Kd_Prov AND A.Kd_Kab_Kota = @Kd_Kab_Kota AND A.Kd_Bidang LIKE @Kd_Bidang AND A.Kd_Unit LIKE @Kd_Unit AND A.Kd_Sub LIKE @Kd_Sub AND A.Kd_UPB LIKE @Kd_UPB
...
--UBAH DATA KIB B
	...
	FROM Ta_KIB_B A WITH(INDEX(IX_Ta_KIB_B)) INNER JOIN
		(
		SELECT A.IDPemda
		FROM
			(
	...
	FROM Ta_KIBBHapus A LEFT OUTER JOIN
		(
		SELECT A.Kd_Riwayat, A.Kd_Id AS Kd_Id, A.IDPemda
		FROM Ta_KIBBR A INNER JOIN
			@no_urut B ON A.Kd_Id = B.Kd_Id AND A.IDPemda = B.IDPemda
		) B ON A.IDPemda = B.IDPemda
