...
/*** fn_Kartu108_BrgE1 - 11052020 - Modified for Ver 2.0.7 [demi@simda.id] ***/
CREATE FUNCTION dbo.fn_Kartu108_BrgE1 (@Tahun varchar(4),@D2 Datetime,@Kd_Prov varchar(3),@Kd_Kab_Kota varchar(3),@Kd_Bidang varchar(3),@Kd_Unit varchar(3), 
    ...
    SELECT A.Tahun, A.IDPemda,A.Kd_Prov,A.Kd_Kab_Kota,A.Kd_Bidang,A.Kd_Unit,A.Kd_Sub,A.Kd_UPB,
	   ...
	   CASE WHEN A.Kondisi >= 3  THEN 4 WHEN B.KD_RIWAYAT IN (9,10,11,12,13)THEN 2 ELSE A.Kd_Aset1 END AS Kd_Aset1, 
		...
		WHEN B.KD_RIWAYAT IN (12,13)THEN 3 WHEN B.Kd_riwayat IN (9) THEN 5 ELSE A.Kd_Aset4 END AS Kd_Aset4, 
	       ...
	       A.No_Register,A.Kd_Ruang,A.Kd_Pemilik,A.Tgl_Perolehan,A.Tgl_Pembukuan,A.Tgl_Dokumen,'' AS No_Dokumen,'' AS Judul,'' AS Pencipta,
	       A.Bahan,A.Ukuran,A.Asal_usul,A.Kondisi,A.Harga,A.Masa_Manfaat,A.Nilai_Sisa,'' AS Keterangan,A.Kd_KA
    FROM Ta_Fn_KIB_E A LEFT OUTER JOIN
