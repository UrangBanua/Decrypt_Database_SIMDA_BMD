...
/*** RptDAFTAR_SMA_PM - 17012017 - Modified for Ver 2.0.7 [demi@simda.id] ***/
CREATE PROCEDURE dbo.RptDaftar_SMA_PM @Tahun varchar(4), @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(3), @D2 Datetime
WITH ENCRYPTION
AS
/*
...
DECLARE @JLap Tinyint SET @JLap = 0
	...
	SELECT A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang,  
		C.Nm_Kab_Kota,C.Nm_Bidang, C.Nm_Unit, C.Nm_Sub_Unit,C.Nm_UPB, C.Nm_UPB AS Lokasi,
		...
		ISNULL (A.Merk,'-') AS Merk, ISNULL (A.Type,'-') AS Type,B.Nilai_Sisa, 
		A.Kondisi, F.Uraian AS UraianKondisi,
	...
	FROM (
		SELECT	A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5, A.Tgl_Perolehan, A.No_Register, A.Kd_KA, A.Harga,
			...
			,ISNULL (A.Merk,'-') AS Merk, A.Type, A.Keterangan, A.Kondisi
		...
		(SELECT NO_BAST, TANGGAL, Kd_prov, Kd_Kab_kota, SUBSTRING (Nm_Kab_kota,12,30) AS Nm_Kab_kota
		 FROM Ta_P3D
		 --WHERE KD_KAB_KOTA = @Kd_Kab_kota2
		) G
	...
	ORDER BY C.Nm_UPB
		--D.Kd_Aset1, D.Kd_Aset2, D.Kd_Aset3, D.Kd_Aset4, D.Kd_Aset5, A.No_Register
