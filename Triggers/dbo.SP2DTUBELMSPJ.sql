...
create PROCEDURE SP2DTUBELMSPJ @Tahun varchar(4), @Kd_Urusan varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @D1 datetime
WITH ENCRYPTION  --- OK
AS
/*
	...
	FROM
		...
		FROM
			...
			WHERE (A.Tahun = @Tahun) AND (A.Kd_Urusan LIKE @Kd_Urusan) AND (A.Kd_Bidang LIKE @Kd_Bidang) AND (A.Kd_Unit LIKE @Kd_Unit) AND (A.Kd_Sub LIKE @Kd_Sub)
				 AND (B.Jn_SPM = 4) AND (C.Tgl_SP2D <= @D1)and (DATEDIFF(day, c.Tgl_SP2D, @D1)>=30)
