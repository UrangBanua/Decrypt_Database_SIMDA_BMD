...
/*** Rpt108KIB_L - 06082019 - Modified for Ver 2.0.7 [demi@simda.id] ***/
CREATE PROCEDURE dbo.Rpt108KIB_L @Tahun varchar(4), @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(10), @D2 Datetime
WITH ENCRYPTION
AS
...
--*/
	...
	SELECT  J.Kd_UPBA, I.Nm_Provinsi, H.Nm_Kab_Kota,
		...
		RIGHT('000000' + CONVERT(varchar, A.No_Register), 6) AS No_Register, YEAR(A.Tgl_Perolehan) AS Tahun, A.Judul, A.Pencipta, A.Bahan, A.Ukuran, 
		 A.Asal_usul, A.Harga, A.Keterangan, 
		J.Nm_Pimpinan, J.Nip_Pimpinan, J.Jbt_Pimpinan, J.Nm_Pengurus, J.Nip_Pengurus, J.Jbt_Pengurus
	FROM fn_Kartu108_BrgL(@D2, @Kd_Prov, @Kd_Kab_Kota, @Kd_Bidang, @Kd_Unit, @Kd_Sub, @Kd_UPB,'%','%','%','%','%','%','%','%',@JLap) A INNER JOIN
