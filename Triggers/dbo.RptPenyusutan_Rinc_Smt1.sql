...
CREATE PROCEDURE dbo.RptPenyusutan_Rinc_Smt1 @Tahun varchar(4), @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), 
...
BEGIN
	...
	SELECT  B.Jndt, A.IDPemda, A.Kd_Prov, A.Kd_Kab_Kota, C.Nm_Kab_Kota, A.Kd_Bidang, C.Nm_Bidang, A.Kd_Unit, C.Nm_Unit, A.Kd_Sub, C.Nm_Sub_Unit, A.Kd_UPB, C.Nm_UPB, A.Kd_Aset_Gab, 
		D.Kd_Aset1, D.Nm_Aset1, D.Kd_Aset2, D.Nm_Aset2, D.Kd_Aset3, D.Nm_Aset3, D.Kd_Aset4, D.Nm_Aset4, D.Kd_Aset5, D.Nm_Aset5, A.No_Register, MONTH (A.Tgl_Perolehan) AS Bulan, 
		YEAR (A.Tgl_Perolehan) AS Tahun, 
       		YEAR(@D1) AS Tahun_Sblm, 
		B.Nilai_Susut1, --B.Nilai_Susut2, B.Akum_susut, 
		B.Nilai_Sisa, B.Sisa_Umur,
       		CONVERT(int, B.Sisa_Umur / 12) AS Thn, 
		(B.Sisa_Umur % 12) AS Bln1,
       		B.Harga AS Nilai_Perolehan, 
                (ISNULL(B.Akum_Susut,0)) - (ISNULL(B.Nilai_Susut2,0)) -(ISNULL(B.Nilai_Susut1,0)) AS AKP_Awal, 
       		(ISNULL(B.Nilai_Susut1,0)) AS Penyusutan1, ((ISNULL(B.Akum_Susut,0))- (ISNULL(B.Nilai_Susut2,0))) AS AKP_Akhir1, 
		--(ISNULL(B.Harga,0) - ISNULL(B.Akum_Susut,0)- ISNULL(B.Nilai_Susut2,0)) AS Nilai_Buku1
		(ISNULL(B.Harga,0)  - (ISNULL(B.Akum_Susut,0) - ISNULL(B.Nilai_Susut2,0))) AS Nilai_Buku1
	FROM (
		...
		FROM   Ta_KIB_B
		WHERE (Kd_Pemilik = CASE WHEN @Kd_Kab_Kota = 0 THEN 11 ELSE 12 END) AND Tgl_Perolehan <= @D1
		) A INNER JOIN
		(
...
BEGIN
	...
	FROM (
		SELECT IDPemda, Kd_Prov, Kd_Kab_Kota, Kd_Bidang, Kd_Unit, Kd_Sub, Kd_UPB, Kd_Aset1, Kd_Aset2, Kd_Aset3, Kd_Aset4, Kd_Aset5, 
	       	CONVERT(varchar, Kd_Aset1)+'.'+ CONVERT(varchar, Kd_Aset2)+'.'+CONVERT(varchar, Kd_Aset3)+'.'+CONVERT(varchar, Kd_Aset4)+'.'+CONVERT(varchar, Kd_Aset5)AS Kd_Aset_Gab,     
	       	No_Register, Tgl_Perolehan, Harga, Kondisi, Kd_KA
		FROM   Ta_KIB_C A
		WHERE (A.Kd_Pemilik = CASE WHEN @Kd_Kab_Kota = 0 THEN 11 ELSE 12 END) AND Tgl_Perolehan <= @D1
		) A INNER JOIN
		(
		SELECT A.Tahun, A.IDPemda, A.Harga, ISNULL(A.Nilai_Susut1,0)AS Nilai_Susut1, ISNULL(A.Nilai_Susut2,0)AS Nilai_Susut2, A.Akum_Susut, A.Nilai_Sisa, A.Sisa_Umur, A.Jndt
...
BEGIN
	...
	WHERE B.Tahun = @Tahun AND A.Kd_Prov LIKE @Kd_Prov AND A.Kd_Kab_Kota LIKE @Kd_Kab_Kota AND A.Kd_Bidang LIKE @Kd_Bidang AND A.Kd_Unit LIKE @Kd_Unit AND 
      		A.Kd_Sub LIKE @Kd_Sub AND A.Kd_UPB LIKE @Kd_UPB AND A.Kd_Aset1 LIKE @Kd_Aset1 AND A.Kd_Aset2 LIKE @Kd_Aset2 AND A.Kd_Aset3 LIKE @Kd_Aset3 AND 
      		A.Kd_Aset4 LIKE @Kd_Aset4 AND D.Kd_Aset1 = 5 AND D.Kd_Aset2 = 20 
	ORDER BY B.Jndt, A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5, A.No_Register
END
