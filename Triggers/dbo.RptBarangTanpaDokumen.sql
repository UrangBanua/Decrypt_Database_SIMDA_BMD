...
/*** RptBarangTanpaDokumen - 05012016 - Modified for Ver 2.0.7 [demi@simda.id] ***/
CREATE PROCEDURE dbo.RptBarangTanpaDokumen @Tahun varchar(4), @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(3), @D2 Datetime
WITH ENCRYPTION
AS 
...
*/
	...
	DECLARE @tmpBI TABLE(Kd_Prov tinyint, Kd_Kab_Kota tinyint, Kd_Bidang tinyint, Kd_Unit tinyint, Kd_Sub smallint, Kd_UPB smallint, Kd_Aset1 tinyint, Kd_Aset2 tinyint, Kd_Aset3 tinyint, Kd_Aset4 tinyint, Kd_Aset5 tinyint, No_Register int, 
		Kd_Pemilik tinyint, Asal_Usul varchar(50), Kondisi varchar(20), Tgl_Perolehan datetime, Tgl_Dokumen datetime, No_Dokumen varchar(100), Harga money, Keterangan varchar(255))
	...
	SELECT A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5, A.No_Register,
		A.Kd_Pemilik, A.Asal_usul, 'Baik' AS Kondisi, A.Tgl_Perolehan, A.Sertifikat_Tanggal AS Tgl_Dokumen, A.Sertifikat_Nomor AS No_Dokumen, A.Harga, A.Keterangan
	...
	INSERT INTO @tmpBI
	SELECT A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5, A.No_Register,
		A.Kd_Pemilik, A.Asal_usul, C.Uraian AS Kondisi, A.Tgl_Perolehan, NULL AS Tgl_Dokumen, A.Nomor_BPKB AS No_Dokumen, A.Harga, A.Keterangan
	...
	WHERE (A.Kd_Prov = @Kd_Prov) AND (A.Kd_Kab_Kota = @Kd_Kab_Kota) AND (A.Kd_Bidang LIKE @Kd_Bidang) AND (A.Kd_Unit LIKE @Kd_Unit) AND (A.Kd_Sub LIKE @Kd_Sub) AND (A.Kd_UPB LIKE @Kd_UPB)
		AND A.Kd_Aset1 = 2 AND A.Kd_Aset2 IN (2,3,10)		

	INSERT INTO @tmpBI
	SELECT A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB, A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5, A.No_Register,
		A.Kd_Pemilik, A.Asal_usul, C.Uraian AS Kondisi, A.Tgl_Perolehan, A.Dokumen_Tanggal AS Tgl_Dokumen, A.Dokumen_Nomor AS No_Dokumen, A.Harga, A.Keterangan
	...
	SELECT M.Nm_Pemda, L.Nm_Provinsi, K.Nm_Kab_KOta, J.Nm_Bidang, I.Nm_Unit, H.Nm_Sub_Unit, G.Nm_UPB, 
		...
		A.Kd_Pemilik, A.Asal_usul, A.Kondisi, A.Tgl_Perolehan, A.Tgl_Dokumen, A.No_Dokumen, A.Harga, A.Keterangan,
	...
	WHERE A.No_Dokumen IS NULL OR A.No_Dokumen LIKE '-'
