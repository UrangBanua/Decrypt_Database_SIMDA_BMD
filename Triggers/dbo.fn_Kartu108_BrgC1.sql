...
/*** fn_Kartu108_BrgC1 - 11052020 - Modified for Ver 2.0.7 [demi@simda.id] ***/
CREATE FUNCTION dbo.fn_Kartu108_BrgC1 (@Tahun varchar(4),@D2 Datetime, @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(4),
	...
	--A.Kd_Aset, A.Kd_Aset0, A.Kd_Aset1, A.Kd_Aset2, A.Kd_Aset3, A.Kd_Aset4, A.Kd_Aset5, A.No_Register, 
	   ...
	   CASE WHEN A.Kondisi >= 3  THEN 4 WHEN B.KD_RIWAYAT IN (9,10,11,12,13)THEN 2 ELSE A.Kd_Aset1 END AS Kd_Aset1, 
		...
		WHEN B.KD_RIWAYAT IN (12,13)THEN 3 WHEN B.Kd_riwayat IN (9) THEN 5 ELSE A.Kd_Aset4 END AS Kd_Aset4, 
           CASE WHEN A.Kondisi >= 3 OR B.KD_RIWAYAT IN (9,10,11,12,13) THEN 3 ELSE A.Kd_Aset5 END AS Kd_Aset5,
	   ...
	   A.No_Register, A.Kd_Pemilik, A.Tgl_Perolehan, A.Tgl_Pembukuan, A.Bertingkat_Tidak, A.Beton_tidak, A.Luas_Lantai, A.Lokasi, A.Dokumen_Tanggal, A.Dokumen_Nomor, 
           ...
           A.Nilai_Sisa, '' AS Keterangan, A.Kd_KA
    FROM Ta_Fn_KIB_C A LEFT OUTER JOIN
	     (SELECT IDPEMDA, Kd_Prov, Kd_Kab_Kota, Kd_Bidang, Kd_Unit, Kd_Sub, Kd_UPB, Kd_Aset8, Kd_Aset80, Kd_Aset81, Kd_Aset82, Kd_Aset83, Kd_Aset84, Kd_Aset85, No_Register, Kd_Riwayat
			...
			AND Kd_Prov = @Kd_Prov AND Kd_Kab_Kota = @Kd_Kab_Kota AND Kd_Bidang LIKE @Kd_Bidang AND Kd_Unit LIKE @Kd_Unit AND Kd_Sub LIKE @Kd_Sub AND Kd_UPB LIKE @Kd_UPB AND
	      		Kd_Aset8 LIKE @Kd_Aset AND Kd_Aset80 LIKE @Kd_Aset1 AND Kd_Aset81 LIKE @Kd_Aset1 AND Kd_Aset82 LIKE @Kd_Aset2 AND Kd_Aset83 LIKE @Kd_Aset3 AND Kd_Aset84 LIKE @Kd_Aset4 AND Kd_Aset85 LIKE @Kd_Aset5 AND No_Register LIKE @No_Register
