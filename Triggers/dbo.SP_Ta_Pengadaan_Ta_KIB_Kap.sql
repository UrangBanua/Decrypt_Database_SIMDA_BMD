...
/*** SP_Ta_Pengadaan_Ta_KIB_Kap - 24032016 - Modified for Ver 2.0.7 [demi@simda.id] ***/
CREATE PROCEDURE [dbo].[SP_Ta_Pengadaan_Ta_KIB_Kap] @Tahun varchar(4), @Kd_Prov varchar(3), @Kd_Kab_Kota varchar(3), @Kd_Bidang varchar(3), @Kd_Unit varchar(3), @Kd_Sub varchar(3), @Kd_UPB varchar(3), @Pil tinyint
WITH ENCRYPTION
AS
/*
...
*/
	SELECT A.Tahun, A.No_Kontrak, A.Kd_Prov, A.Kd_Kab_Kota, A.Kd_Bidang, A.Kd_Unit, A.Kd_Sub, A.Kd_UPB,
			A.Tgl_Kontrak, B.Tgl_SP2D, A.Keterangan, A.Kd_Posting, A.Total, B.Penunjang, A.No_Bast, A.Tgl_BAST
	FROM
		(
		...
		FROM Ta_Pengadaan_Rinc A INNER JOIN
			Ta_Pengadaan B ON A.Tahun = B.Tahun AND A.No_Kontrak = B.No_Kontrak INNER JOIN
				Ta_Pengadaan_Bast C ON B.Tahun = C.Tahun AND A.No_Kontrak = C.No_Kontrak
